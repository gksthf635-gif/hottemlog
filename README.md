...
메인 상품은 `핫템 바로가기`에서 한 번만 표시하며 카테고리 필터를 제공합니다. 상품명 클릭은 모달(모바일 하단 시트)을 열고, 팝업의 쿠팡 CTA에서만 기존 `/go`로 이동합니다. 검색·영상 상세도 같은 팝업을 사용합니다. 직접 상품 상세 URL과 관리자/DB 구조는 그대로 유지합니다. 테스트는 실제 공개 데이터 읽기 및 빈 배열만 사용하고 가짜 상품이나 클릭을 생성하지 않습니다. 상세 결과: `PUBLIC_REBUILD_V3_REPORT.md`.
